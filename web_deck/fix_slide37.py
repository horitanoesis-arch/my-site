...
# New Content (Aligned SVG, Icon, Slow Speed)
new_content = """<div class="flex-1 px-12 pb-8 grid grid-cols-2 gap-8 relative items-center">
                ...
                <div class="relative h-full flex flex-col items-center justify-center p-6 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-300 overflow-hidden">
                  ...
                  <div class="absolute inset-0 pointer-events-none overflow-hidden">
                    ...
                    <div class="absolute top-10 left-1/2" style="animation: drop-fade 2s infinite">
                  ...
                  <div class="mt-8 bg-red-50 text-red-500 text-xs font-bold px-3 py-1 rounded border border-red-200">
                    <i class="fas fa-times-circle mr-1"></i>下まで届かない...
                  </div>
                </div>
                ...
                <div class="relative h-full flex flex-col items-center justify-center p-6 bg-blue-50/50 rounded-3xl border-2 border-blue-200 shadow-xl overflow-hidden">
                  <div class="absolute top-4 left-6 bg-blue-600 text-white px-3 py-1 rounded-full text-sm font-bold z-10 shadow-md">
                    <i class="fas fa-check mr-1"></i>Zenkou-Saiモデル
                  </div>
                  
                  <!-- Money Shower Animation (Full Coverage) -->
                  <style>
                    @keyframes shower-down-full {
                    ...
                    .coin-shower {
                      position: absolute;
                      top: -50px;
                      animation: shower-down-full linear infinite;
                    }
                  </style>
                  ...
                  <div class="relative flex flex-col items-center w-full max-w-md mt-8 scale-90 z-10">
                    ...
                    <div class="relative z-10 flex gap-8 w-full justify-between">
                      <!-- Node 1 -->
                      <div class="w-10 h-10 bg-yellow-50 border-2 border-yellow-400 rounded flex items-center justify-center shadow-md relative transform hover:scale-110 transition-transform">
                  ...
                  <div class="mt-8 bg-blue-100 text-blue-600 text-xs font-bold px-3 py-1 rounded border border-blue-200">
                    <i class="fas fa-check-circle mr-1"></i>末端までしっかり還元！
                  </div>
                </div>
              </div>"""
...
# I know the start and end markers from viewing the file
# Start: <div class="flex-1 px-12 pb-8 grid grid-cols-2 gap-8 relative items-center">
# End: <div class="mt-8 bg-blue-100 text-blue-600 text-xs font-bold px-3 py-1 rounded border border-blue-200">...</div></div></div>

# Finding the section to replace by broad index is safer given I have line numbers from previous step
start_marker = '<div class="flex-1 px-12 pb-8 grid grid-cols-2 gap-8 relative items-center">'
# End marker is risky.
# Instead, I'll match the large block I read previously.
# But python string matching over 200 lines is brittle with whitespace.
# I'll use regex to separate the file into (before) + (target_section) + (after)

import re

# We will look for the unique start of the section and the unique end of the slide section (slide 37 ends soon after)
# The slide 37 block is inside <section data-slide="37">...</section>
# I will find the slide 37 container and replace its content body.
...
# If regex fails, I'll print error.
